...
app.get("/api/informeBrigadistaUbicacion",(req,res)=>{
  //Consulta a la base de datos usando la sentencia sql que viene por parámetro
  conexion.query("SELECT pais,departamento,ciudad, COUNT(nombreBrigadista) AS cantidad FROM vbrigadista GROUP BY pais,departamento,ciudad ORDER BY pais,departamento,ciudad  asc", function (error, filas) {
    //Si existe un error nos devuelve el error detectado
    if(error){
      throw error
  }else{
